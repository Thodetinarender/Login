...
<input type="hidden" id="status" value="<%= request.getAttribute("status") %>">
    ...
    if(status == "success"){
    	swal("congrats","Account Craeted Successfully","success");
    }
    
     if(status=="InavlidUrname"){
    	swal("Sorry","Please enter your name","error");
    }
    
     if(status=="InvaliUremail"){
    	swal("Sorry","Please enter your email","error");
    }
     if(status=="InvalidUrpassword"){
    	swal("Sorry","Please enter your password","error");
    }
     if(status=="InvalidConformPassword"){
    	swal("Sorry","Please enter correct conform password","error");
    }
     if(status=="InvalidUrmobile"){
    	swal("Sorry","Please enter your mobile number","error");
    }
      if(status=="InvalidUrmobileLength"){
    	  swal("Sorry","please enter 10 digits of number","error");
      }
    </script>
